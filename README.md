...
- `command-topic` / `result-topic-job` / `result-topic-log` / `audit-topic` /
  `heartbeats-topic` 토픽은 `KAFKA_AUTO_CREATE_TOPICS_ENABLE=true`로 자동 생성된다.
...
| `1` | 부팅 실패 또는 결과 토픽(`result-topic-job`/`result-topic-log`) / `audit-topic` publish 실패 (fail-fast) | **재기동 필수** — last committed offset부터 redeliver되어 at-least-once 보장 |
...
| 흐름 | 토픽 | 발행/소비 |
|---|---|---|
| 시작 알림 + 등록 | `audit-topic` (`AGENT_STARTED`) | 발행 |
| 명령 수신 | `command-topic` | consume (`script-agent-<agent_id>` 그룹) |
| Job 실행 결과 (SCRIPT_JOB) | `result-topic-job` | 발행 |
| Job 실행 결과 (LOG_JOB) | `result-topic-log` | 발행 |
| 감사 이벤트 | `audit-topic` (`JOB_EXECUTED`) | 발행 |
| 종료 알림 | `audit-topic` (`AGENT_STOPPED`) | 발행 |
| Liveness | `heartbeats-topic` | OTel Collector가 OTLP→Kafka 재발행 |
...
- **발행 순서**: 결과 토픽(job_type별 `result-topic-job`/`result-topic-log`) 먼저,
  성공 시 `audit-topic` (JOB_EXECUTED).
...
| `KAFKA_TOPIC_COMMANDS` | `command-topic` | BE→Agent 명령 토픽 |
| `KAFKA_TOPIC_RESULT_JOB` | `result-topic-job` | Agent→BE SCRIPT_JOB 결과 토픽 (T4-2 분리) |
| `KAFKA_TOPIC_RESULT_LOG` | `result-topic-log` | Agent→BE LOG_JOB 결과 토픽 (T4-2 분리) |
| `KAFKA_TOPIC_AUDIT_EVENTS` | `audit-topic` | Agent→BE 감사 이벤트 토픽 |
...
2. `audit-topic` 토픽 consume:
   ```sh
   docker compose -f ../infra/docker-compose.yml exec kafka \
     kafka-console-consumer --bootstrap-server localhost:9092 \
     --topic audit-topic --from-beginning
   ```
   → `AGENT_STARTED` 이벤트 확인.
3. `command-topic` 토픽에 SCRIPT_JOB 또는 LOG_JOB JSON 1건 produce
   (`target_agent_id`는 위에서 확인한 agent_id).
4. 결과 토픽 consume → 결과 페이로드 확인 (SCRIPT_JOB이면 `result-topic-job`,
   LOG_JOB이면 `result-topic-log`).
5. `audit-topic` 다시 → `JOB_EXECUTED` 확인.
6. `heartbeats-topic` 토픽 consume → 10초 주기로 OTLP heartbeat 메시지 확인
