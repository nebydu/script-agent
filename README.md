...
- `command-topic` / `job-results` / `audit-topic` / `heartbeats-topic` 토픽은
...
| `1` | 부팅 실패 또는 `job-results` / `audit-topic` publish 실패 (fail-fast) | **재기동 필수** — last committed offset부터 redeliver되어 at-least-once 보장 |
...
- **발행 순서**: `job-results` 먼저, 성공 시 `audit-topic` (JOB_EXECUTED).
