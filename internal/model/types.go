...
// 정의한다. Agent가 직접 다루는 토픽(commands / job-results /
// audit-events)만 포함하며, heartbeats는 OTel Collector가 발행하므로
...
// JobType은 commands / job-results 페이로드의 job_type 필드값이다.
// spec §5.1, §5.2.
type JobType string
...
// AuditAction은 audit-events 페이로드의 action 필드값이다.
...
// ActorType은 audit-events actor.type 필드값이다.
...
// TargetType은 audit-events target.type 필드값이다. spec §5.3.4.
...
// EventResult는 audit-events result 필드값이다. spec §5.3.4.
//
