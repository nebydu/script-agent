...
> **근거(provenance)**: 셋업 원 브리프(HANDOFF §5)의 초기 우선순위는 "코드 → 데모 spec v0.2.1 → 통합본"이었으나, hub가 커밋 `9b7288f`로 통합본 중심 재조정을 먼저 단행했다. 폴리레포 오케스트레이션 일관성을 위해 위 순서로 정렬했다 — **사용자 명시 승인(2026-05-29)**. 원 브리프 acceptance 기준을 의도적으로 supersede한 것이며, 데모 spec v0.2.1은 버린 것이 아니라 #4 Phase 0 회귀 가드로 유지된다.
...
- **script-agent 루트 AGENTS.md / CLAUDE.md 파이프라인 자동 갱신 금지**: sub-agent 파이프라인이 임의로 건드리지 않는다. 단, **사람이 명시적으로 지시한 수동 갱신은 예외**이며 근거를 커밋 메시지에 남긴다(예: 2026-05-29 통합본 중심 정렬, hub `9b7288f`에 맞춤 — 사용자 승인).
...
- **실행 방식**: `settings.json`은 exec form으로 `.claude/hooks/git-bash.cmd` shim을 호출한다. 이 shim은 한 줄짜리 Windows `cmd` 스크립트로 `%ProgramFiles%\Git\bin\bash.exe`를 동적으로 expand해 실행한다. 표준 Git for Windows 설치(기본 경로)에서 그대로 동작하며, 비표준 설치는 shim 한 줄만 수정한다. **PATH 의존 없음** — Windows PATH상 `bash`가 WSL(`C:\Windows\System32\bash.exe`)로 먼저 잡혀도 영향받지 않는다.
