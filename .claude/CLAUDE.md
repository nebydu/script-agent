...
> **근거(provenance)**: 셋업 원 브리프(HANDOFF §5)의 초기 우선순위는 "코드 → 데모 spec v0.2.1 → 통합본"이었으나, 이전 하네스 정렬 과정에서 통합본 중심 재조정이 반영됐고 형제 repo `hub`도 현재 통합본-우선 정렬 상태다. 폴리레포 오케스트레이션 일관성을 위해 위 순서로 정렬한 것은 **사용자 명시 승인(2026-05-29)** 사항이다. 원 브리프 acceptance 기준을 의도적으로 supersede한 것이며, 데모 spec v0.2.1은 버린 것이 아니라 #4 Phase 0 회귀 가드로 유지된다.
...
- **work-id 바인딩 계약**: 파이프라인 시작 시 `<work-id>`를 **명시적으로 확정**하고, analyzer → implementer → tester → reviewer/spec-guardian 모든 호출에 **동일한 work-id를 전달**한다. work-id가 불명확하면 대화 맥락으로 추론하지 말고 멈춰 사람에게 확인한다. (Codex 게이트(plugin Stop hook)는 git diff 기반 경량 게이트라 work-id를 받지 않으며 handoff 정합성을 검사하지 않는다 — handoff 검사는 analyzer/spec-guardian 책임.)
- **monitoring-meta 버전 핀**: 작업 spec(`../monitoring-meta/handoff/<work-id>-script-agent.md`) 헤더에 `기준 monitoring-meta commit: <hash>`(전체 또는 단축 SHA)가 **반드시 명시**돼야 한다. analyzer가 파이프라인 첫 행동으로 `git -C ../monitoring-meta rev-parse HEAD` 결과와 대조하며, **불일치하면 분석을 진행하지 않고 멈춰 사람에게 보고**한다(`blockers`에 drift 사실 명시, `status: blocked`). 정본(monitoring-meta)이 spec 작성 시점 이후 변동된 상태에서 그 spec을 기준으로 분석·구현하지 않기 위한 안전장치다.
...
| agent | Write 권한 | 핵심 |
|---|---|---|
| analyzer | `analysis/`만 | 종합 분석, 미결정 사안 발견 시 사람 호출 게이트. `docs/`는 정본 사본 보호로 쓰기 금지(settings.json deny와 정합) |
...
- Codex 게이트는 **`monitoring-harness@monitoring-harness` plugin**(project scope, `enabledPlugins`)이 제공하는 Stop hook으로 작동한다. 이전의 native `.claude/hooks/codex-gate.sh` + `settings.json`의 Stop 블록은 **2026-06-02 plugin 전환**으로 제거됐다(런타임 동등성은 plugin `shared/hooks/h2b-validation.md`에서 검증).
