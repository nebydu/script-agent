...
PROMPT="script-agent(Go) 코드 변경 리뷰. 다음을 read-only로만 검토하고 codex-schema.json 형식의 JSON으로만 응답하라: (1) 버그·회귀 가능성 (2) Phase 0 데모 spec docs/monitoring-demo-message-spec-v0.2.1.md와의 명세 불일치 (3) 데모 spec v0.2.1 §6.2 Job 실행 정책 불변식 6종 위반 — ① at-least-once(결과/감사 발행 완료 전 Kafka offset commit 금지) ② fail-fast(job-results/audit-events publish 실패 시 exit 1 경로) ③ 발행 순서(job-results 먼저, audit-events JOB_EXECUTED 나중; results 실패 후 audit 시도 금지) ④ valid_until 지난 명령 silent skip ⑤ 단일 consumer goroutine 직렬 처리(동일 schedule 재진입 불가) ⑥ LOG_JOB file_id 변경·size shrink 시 재시작 (4) 테스트 누락. 빌드 검증 기준은 'go build ./...', 'go test ./...', 'go vet ./...'. 위상 주의: envelope.md는 Phase 1 목표이고 script-agent 코드는 Phase 0 위상이므로, envelope consumer측 동작(중복검사/trace복원) 미구현은 위반이 아니다. 데모 단계는 Kafka 통합 테스트가 없으므로(모델/유틸 단위 테스트만), Kafka 실연동 테스트 부재 자체를 위반으로 보고하지 마라."
