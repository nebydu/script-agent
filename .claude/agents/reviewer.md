...
2. **fail-fast 위반** — 결과 토픽(`result-topic-job`/`result-topic-log`)/`audit-topic` publish 실패 시 `exit 1`(consumer self-terminate) 아닌 경로로 진행.
3. **발행 순서 위반** — 결과 토픽(`result-topic-job`/`result-topic-log`)보다 `audit-topic`(JOB_EXECUTED)를 먼저 발행하거나, results 실패 후 audit을 시도. (정상: `internal/job/dispatcher.go`가 results.Publish → auditor.JobExecuted)
